...
def senator_reports(client: requests.Session, limit) -> List[List[str]]:
    """ Return a limited number of results from the periodic transaction reports API. """
    token = _csrf(client)
    idx = 0
    reports = reports_api(client, idx, token)
    all_reports: List[List[str]] = []
    while len(reports) != 0 and len(all_reports) < limit:
